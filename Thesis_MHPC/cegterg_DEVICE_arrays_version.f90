...
SUBROUTINE cegterg( h_psi_ptr, s_psi_ptr, uspp, g_psi_ptr, &
  ...
  COMPLEX(DP), ALLOCATABLE, DEVICE :: hc_comp(:,:,:), sc_comp(:,:,:), vc_comp(:,:,:) !! M.Iovine - we define the corresponding 3D arrays
  ...
  !!
  IF (.NOT. ALLOCATED(hc_comp)) ALLOCATE(hc_comp(nbase,nbase_max,n_k))
  IF (.NOT. ALLOCATED(sc_comp)) ALLOCATE(sc_comp(nbase,nbase_max,n_k))
  IF (.NOT. ALLOCATED(vc_comp)) ALLOCATE(vc_comp(nbase,nbase_max,n_k))
 ...
 !end do
  ...
  !!! Diagonal elements (we base them on the maximum element      of the reduced matrices --> we multiply for 10^5 for faster convergen     ce
     ...
     !$acc parallel loop collapse(2) present(hc, sc, vc) deviceptr( hc_comp, sc_comp, vc_comp)
     do i = 1, nbase_max
         do j = 1, nbase_max
     ...
     !!$acc host_data use_device(vc(1:nbase_max,1:nbase_max), ew(1:nbase_max))
     CALL start_clock( 'cegterg:diag' )
     ...
     !$acc parallel loop present(ew) deviceptr(ew_comp) async(async_id)
     DO i = 1, nbase_max
     ...
     !$acc parallel loop collapse(2) present(vc) deviceptr(vc_comp)
     DO j = 1, nbase_max
        DO i = 1, nbase_max
...
! NB: must not call mp_sum over inter_bgrp_comm here because it is done later to the full correction
  ...
  !!DEBUGG trying to solve the NaN issue:
  DEALLOCATE( vc_comp )
  DEALLOCATE( hc_comp )
  DEALLOCATE( sc_comp )
  !!!
