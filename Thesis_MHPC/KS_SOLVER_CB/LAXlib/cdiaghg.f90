...
#define VARTYPE ALLOCATABLE
  ...
  TYPE(c_devptr), ALLOCATABLE :: arr_of_ptr_h_d(:), arr_of_ptr_s_d(:)
  ...
  COMPLEX(DP), ALLOCATABLE :: min_col_arr(:) !! M.Iovine - column array corresponding to ind_min
  ! various work space
  !
  ! Temp arrays to save H and S.
  REAL(DP), VARTYPE    :: h_diag_d(:), s_diag_d(:)
  ...
  istat_cublas = cublasCreate(cublas_handle)
#undef VARTYPE
  ...
  IF ( me_bgrp == root_bgrp ) THEN
      ...
      IF( info /= 0 ) CALL lax_error__( ' cdiaghg_gpu ', ' cannot allocate arr_of_ptr_h ', ABS( info ) )
      !! M.Iovine - allocation of column array corresp. to index of max. eigenvalue:
      ALLOCATE(min_col_arr(n),STAT = info)
      IF( info /= 0 ) CALL lax_error__( ' cdiaghg_gpu ', ' cannot allocate min_col_arr ', ABS( info ) )
...
!$cuf kernel do(3) <<<*,*,0,laxlib_cuda_stream>>> !!!! M.Iovine - Modified the loops nested from 2 to 3 --> so we changed kernel do(2) to kenel do(3) :
    ...
    IF ( info /= CUSOLVER_STATUS_SUCCESS ) CALL lax_error__( ' cdiaghg_gpu ', 'cublasZtrsmBatched-LEFT',  ABS( info ) )
    ...
    info = cublasZtrsmBatched(cublas_handle, CUBLAS_SIDE_RIGHT, &
           ...
           n, n, alpha, arr_of_ptr_s_d, ldh, arr_of_ptr_h_d, ldh, n_k)    
    IF ( info /= CUSOLVER_STATUS_SUCCESS ) CALL lax_error__( ' cdiaghg_gpu ', 'cublasZtrsmBatched-RIGT',  ABS( info ) )
      !
    ...
    info = cublasZtrsmBatched(cublas_handle, CUBLAS_SIDE_LEFT, &
           CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_C, CUBLAS_DIAG_NON_UNIT, &
           n, n, alpha, arr_of_ptr_s_d, ldh, arr_of_ptr_h_d, ldh, n_k)
    IF ( info /= CUSOLVER_STATUS_SUCCESS ) CALL lax_error__( ' cdiaghg_gpu ', 'cublasZtrsmBatched-eigenvectors',  ABS( info ) )
...
!$cuf kernel do(3) <<<*,*,0,laxlib_cuda_stream>>>
      ...
      DEALLOCATE(min_col_arr) !! M.Iovine - we deallocate min_col_arr   
#else
      CALL dev%release_buffer( work_d,  info )
      CALL dev%release_buffer( h_bkp_d, info )
      CALL dev%release_buffer( s_bkp_d, info )
