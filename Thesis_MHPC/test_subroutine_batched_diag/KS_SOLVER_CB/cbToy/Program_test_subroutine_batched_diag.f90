...
PROGRAM script_diag_test
 ...
 !! We initializethe cusolver handle:
 call initialize_cusolver_handles(i_batch)
 !! We initalize the clock for the GPU execution:
 call init_clocks(.true.)

    !! Allocation of the arrays:
    ALLOCATE(h_d(ldh,n,i_batch))
    ALLOCATE(s_d(ldh,n,i_batch))
    ALLOCATE(v_d(ldh,n,i_batch))
    ALLOCATE(e(n))
    ALLOCATE(e_d(n,i_batch))
    ALLOCATE(s(n,n))
    ALLOCATE(h(n,n))
    ALLOCATE(v(n,n))
    ALLOCATE(Sv(n))
    ...
    do k = 1, n
        ...
        IF (k < n) v(k+1,k) = (0.5D0, 0.D0) !plus overlap to make GS nontrivial
    end do
    ...
    ! sanity check: V^H S V should be identity
    ...
    DO i = 1, n
       DO j = 1, n
          DO k = 1, n
             h(i,j) = h(i,j) + lambda(k) * SUM(s(i,:) * v(:,k)) * CONJG(SUM(s(j,:) * v(:,k)))
          END DO
       END DO
    END DO
    !!!!!!!!!!!
    
    !! We copy the h and s 2d arrays on the device: --> WARNING !!! WE NEED TO USE AN enter data, NOT A declare data !!!!
    !$acc enter data copyin(h, s, v)
    ...
    PRINT *, 'BEFORE THE DIAGHG!'
    

    !$acc host_data use_device(h_d, s_d, e_d, v_d)
    CALL diaghg(n,m,h_d,s_d,ldh,e_d,v_d,i_batch,0,0,0)
    !$acc end host_data

    PRINT *, 'AFTER THE DIAGHG!'
    ...
    !!! We sync results from GPU back to CPU host before printing
    !$acc update host(e, v)
    ...
    DO i = 1, n
       ip = SUM(CONJG(v(:,i)) * MATMUL(s, v(:,i))) !! We multiply the complex conjugate of the i-th eigenvector for the matrix multipl. of S
                                                   !! and the i-th eigenvector --> it should be 1 !!
