...
template <typename T> class CMesh{
    ...
    void print_funct(std::ofstream& os, double* field_d); //Print function
...
void CMesh<T>::print_funct(std::ofstream& os, double* field_d){
...
void CMesh<T>::jacobi(const size_t& max_steps, const size_t& PrintInterval){
    ...
    //Pointers initialization for GPU:
    double* field_d = field.data();
    double* new_field_d = new_field.data();
    double* ghost_up_d = ghost_up.data();
    double* ghost_dn_d = ghost_down.data();
    ///

    //Pointers initialization for GPU:
    double* send_buff_loc_prev1 = send_buff_loc_prev.data();
    double* send_buff_loc_next1 = send_buff_loc_next.data();
    ///

    #pragma acc data copy(field_d[0:size_d*nloc_d], new_field_d[0:size_d*nloc_d], send_buff_loc_prev1[0:size_d], send_buff_loc_next1[0:size_d], ghost_dn_d[0:size_d], ghost_up_d[0:size_d])
    {
	 for(int k = 0; k < (int)max_steps; k++){
        ...
        //ATTENTION!!!!!: IN ORDER TO MAKE THE host_data use_device to work, we need to be sure THAT MPI IS CUDA AWARE!!:
        //We need to make sure that MPI is capable of sending/receiving data directly from/to the Device
        //We need to make MPI capable of de-reference a Device (GPU) pointer!!!!
        //SO MPI MUST BE "AWARE" THAT THE GPU (AND ITS VARIABLES) ARE PRESENT!!!!
        #pragma acc host_data use_device(/*field1, new_field1, send_buff_loc_prev1, send_buff_loc_next1,*/ ghost_dn_d, ghost_up_d)
	    {
		///////////
		for(int i=0; i < size; i++){
          		send_buff_loc_prev1[i] = field_d[i];
      		}

      		for(int i=0; i < size; i++){
          		send_buff_loc_next1[i] = field_d[size*(nloc-1) + i];;
      		}

      		//We introduce 2 Sendrecv: one to send up and one to send down
      		//Send up:
      		MPI_Sendrecv(send_buff_loc_prev1,size,MPI_DOUBLE,prev,0,ghost_dn_d,size,MPI_DOUBLE,next,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);

      		//Send down:
      		MPI_Sendrecv(send_buff_loc_next1,size,MPI_DOUBLE,next,1,ghost_up_d,size,MPI_DOUBLE,prev,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		...
		#pragma acc host_data use_device(field_d, new_field_d)
		{
		    //We swap the pointers on the Device:
		    /*T* temp = field_d;
            field_d = new_field_d;
            new_field_d = temp;*/ //The two vectors,one for the current iteration and one for the previous one,are swappedin order to make the 
                                        //pointers of the new one be associated to the old field and viceversa.
	        std::swap(field_d, new_field_d);
        }
	    
        std::swap(field_d, new_field_d);
        //#pragma acc update host(field_d[0:size_d*nloc_d])
        //#pragma acc update host(new_field_d[0:size_d*nloc_d])
        //field_d = field.data();
        //new_field_d = new_field.data();
