...
SUBROUTINE laxlib_cdiaghg_gpu(cublas_handle, n, m, h_d, s_d, ldh, e_d, v_d, n_k, me_bgrp, root_bgrp, intra_bgrp_comm)
  !----------------------------------------------------------------------------
  !!
...
#define VARTYPE ALLOCATABLE
  ...
  COMPLEX(DP), INTENT(INOUT) :: h_d(ldh,n,n_k)
  !! matrix to be diagonalized, allocated on the GPU
  COMPLEX(DP), INTENT(INOUT) :: s_d(ldh,n,n_k)
  !! overlap matrix, allocated on the GPU
  !!!! M.Iovine - Batched change:
  REAL(DP), INTENT(OUT) :: e_d(n,n_k)
  !!!!
  !! eigenvalues, , allocated on the GPU
  COMPLEX(DP),  INTENT(OUT) :: v_d(ldh,n,n_k) !!!! M.Iovine - v_d becomes a 3D array!!!
